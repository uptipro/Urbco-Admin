...
const Users = () => {
  ...
  const actionHandler = async () => {
    try {
      setLoad(true);
      let type = clickedUser.status === "active" ? "deactivate" : "activate";
      ...
      <div>
        {(user_details.user_type === "admin" ||
          (user_details.role_id &&
            user_details.role_id.permissions.includes("create-user"))) && (
          <button
            className="main-btn"
            onClick={() => {
              setModalType("create");
            ...
            user_list.users && (
              <>
                  ...
                  <tbody>
                    {user_list.users.map((user) => (
                      <tr key={user._id}>
                        ...
                        <td className="wide">{user.email}</td>
                        <td>{user.mobile}</td>
                        <td className="wide">
                          <span className="me-2">
                            {user.role_id ? user.role_id.name : "Role not set"}
                          </span>
                          {(user_details.user_type === "admin" ||
                            (user_details.role_id &&
                ...
                {user_list.meta && user_list.meta.pages > 1 && (
                  <>
                    <Pagination
                      currentPage={Number(user_list.meta.page)}
                      totalCount={Number(user_list.meta.total)}
                      pageSize={10}
