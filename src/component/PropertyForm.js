...
const PropertyForm = ({ propertyDetails }) => {
  ...
  useEffect(() => {
    if (propertyDetails && propertyDetails.id) {
      ...
      setCapitalAppreciation(propertyDetails.capital_appreciation_percent);
      ...
      try {
        ...
        if (propertyDetails && propertyDetails.id) {
          ...
          if (res && res.data && res.data.ref) {
            try {
              await propertyService.sendToBuyops(
                user_details.access_token,
                res.data.ref,
              );
              toast.success("Property created and linked to BuyOps.", {
                position: "top-right",
              });
            } catch (sendErr) {
              toast.error(
                "Property created but could not be linked to BuyOps. Use 'Send to BuyOps' from the edit page.",
                { position: "top-right" },
              );
            }
          }
  ...
  const statusHandler = async () => {
    if (window.confirm("Are you sure you want to upgrade this status?")) {
      if (roofingDate && constructionEnd && constructionStart) {
        ...
        alert("You need to provide the construction and roofing dates.");
      }
    }
  };
          ...
          <TabContent activeTab={activeTab}>
                      ...
                      <div className="text-end">
                        <div className="upload-btn-wrapper">
                          <button className="btn">
                            <BsCloudUpload color="#852472" size={25} />
                          </button>
                          <input
                            type="file"
                            name="myfile"
                          ...
                          <div className="mt-2">
                            ...
                            >
                              You have an Image Url?
                            </a>
                          </div>
                          {previewSource && (
                            <div className="mt-2" onClick={handleSubmitFile}>
                              <button className="main-btn" disabled={uploading}>
                                {uploading ? "Hold on.." : "Upload"}
                              </button>
                            </div>
                          )}
                        </div>
                  ...
                  <div className="uploaded mt-3">
                    {haveUrl && (
                      <div className="div-haveurl">
                        ...
                        />
                        <button className="main-btn" onClick={submitUrl}>
                          Submit
                        </button>
                      </div>
                    )}
                ...
                <div className="row">
                  <div className="col-lg-6 mb-3">
                    ...
                    <input
                      ...
                      onChange={(e) => setFractionPerUnit(e.target.value)}
                      type="number"
                    />
                    <label>
                    ...
                    <CurrencyInput
                      ...
                      value={costFractions}
                      decimalsLimit={2}
                      onValueChange={(value, name) => setCostFractions(value)}
                      prefix={"₦ "}
                    />
                  </div>
                  <div className="col-lg-6 mb-3">
                    ...
                    <CurrencyInput
                      ...
                      value={costUnit}
                      decimalsLimit={2}
                      onValueChange={(value, name) => setCostUnit(value)}
                      prefix={"₦ "}
                      disabled={true}
                    />
                    <label>
                    ...
                    <CurrencyInput
                      ...
                      value={totalPrice}
                      decimalsLimit={2}
                      onValueChange={(value, name) => setTotalPrice(value)}
                      prefix={"₦ "}
                      disabled={true}
                    />
                  </div>
              ...
              <div className="form pt-3">
                <div className="row">
                  <div className="col-lg-6">
                    ...
                    {features.length > 0 &&
                      features.map((f) => (
                        <div className="row" key={f.feature}>
                          <div className="col-6">
                            ...
                            <div className="row">
                              ...
                              <div className="col-2">
                                <div className="check-btn">
                                  <button
                                    className="shadow-sm"
                                    onClick={() => removeFeature(f.feature)}
                                  >
                                    <RxCross2 />
                                  </button>
                                </div>
            ...
            <TabPane tabId={"5"}>
              <div className="form pt-3">
                <div className="row">
                  <div className="col-lg-6 mb-3">
                    <label>Construction Stage Discount (%)</label>
                    <input
                      type="number"
                      value={cspD}
                      onChange={(e) => setCspD(e.target.value)}
                      disabled={
                        propertyDetails && propertyDetails.status !== "pending"
                          ? true
                          : false
                      }
                    />
                    <label>Construction Stage Volume Available (%)</label>
                    <input
                      type="number"
                      value={cspV}
                      onChange={(e) => setCspV(e.target.value)}
                      disabled={
                        propertyDetails && propertyDetails.status !== "pending"
                          ? true
                          : false
                    ...
                    />
                    <label>Offplan Bullet Discount (%)</label>
                    <input
                      type="number"
                      value={opbpD}
                      onChange={(e) => setOpbpD(e.target.value)}
                      disabled={
                        propertyDetails && propertyDetails.status !== "pending"
                          ? true
                          : false
                    ...
                    />
                    <label>Offplan Bullet Volume Available (%)</label>
                    <input
                      type="number"
                      value={opbpV}
                      onChange={(e) => setOpbpV(e.target.value)}
                      disabled={
                        propertyDetails && propertyDetails.status !== "pending"
                          ? true
                          : false
                    ...
                    />
                    <label>Offplan Tranche Discount (%)</label>
                    <input
                      type="number"
                      value={optpD}
                      onChange={(e) => setOptpD(e.target.value)}
                      disabled={
                        propertyDetails && propertyDetails.status !== "pending"
                          ? true
                          : false
                    ...
                    />
                    <label>Offplan Tranche Volume Available (%)</label>
                    <input
                      type="number"
                      value={optpV}
                      onChange={(e) => setOptpV(e.target.value)}
                      disabled={
                        propertyDetails && propertyDetails.status !== "pending"
                          ? true
                          : false
                    ...
                    <input
                      type="number"
                      value={optpStage}
                      onChange={(e) => setOptpStage(e.target.value)}
                      disabled={
                        propertyDetails && propertyDetails.status !== "pending"
                          ? true
                          : false
                    ...
                    />
                    <label>Offplan Tranche Percent Split</label>
                    <input
                      type="text"
                      placeholder="Should be comma separated"
                      value={optpPercent}
                      onChange={(e) => setOptpPercent(e.target.value)}
                      disabled={
                        propertyDetails && propertyDetails.status !== "pending"
                          ? true
                          : false
                  ...
                  <div className="col-lg-6 mb-3">
                    ...
                    />
                    <label>Rental Yield Assumption (%)</label>
                    <input
                      type="number"
                      value={rentals.yield_assumption_percent}
                      onChange={(e) =>
                        setRentals({
                          ...rentals,
                          yield_assumption_percent: e.target.value,
                        })
                      }
                    />
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setRentals({
                          ...rentals,
                          first_dividend_date: e.target.value,
                        })
                      }
                    />
                    <label>Capital Appreciation - Annual (%)</label>
                    <input
                      type="number"
              ...
              <div className="form pt-3">
                <div className="row">
                  <div className="col-lg-6 mb-3">
                    ...
                    {constructionStart && (
                      <p className="value">
                        {dateFormat(constructionStart, "mmmm dS, yyyy")}
                      </p>
                    )}
                    <input
                      type="date"
                    ...
                    {constructionEnd && (
                      <p className="value">
                        {dateFormat(constructionEnd, "mmmm dS, yyyy")}
                      </p>
                    )}
                    <input
                      type="date"
                    ...
                    {roofingDate && (
                      <p className="value">
                        {dateFormat(roofingDate, "mmmm dS, yyyy")}
                      </p>
                    )}
                    <input
                      type="date"
                      value={roofingDate}
                      onChange={(e) => setRoofingDate(e.target.value)}
                    />
                    {propertyDetails &&
                      propertyDetails.status &&
                      propertyDetails.status !== "completed" && (
                        <div className="row align-items-center">
                          ...
                          <div className="col-12 check-btn">
                            ...
                            >
                              Update Status to{" "}
                              {propertyDetails.status === "design"
                                ? "Construction"
                                : propertyDetails.status === "construction"
                                  ? "Completed"
                                  : "Furnished"}
                            </button>
                          </div>
